...
recompute a link in the chain in a reasonable time even once, let
...
in mind that they have have a significant fraction of the world's
...
that? No. A hash can be anything with the properties loosely described
of "one way" and "non-linear". I used `hashlib.md5` because I used a
similar library in my original code in another language that had fewer
choices.
